...
def build_graphs(P_np: np.ndarray, n: int, n_pad: int=None, edge_pad: int=None, stg: str='fc', sym: bool=True,
                 thre: int=0, namelist=None) -> Tuple[np.ndarray, np.ndarray, np.ndarray, int]:
    r"""
    ...
    assert stg in ('fc', 'tri', 'near', 'gabriel'), 'No strategy named {} found.'.format(stg)

    if stg == 'tri':
        A = delaunay_triangulate(P_np[0:n, :])
    elif stg =='gabriel':
        A = gabriel_graph(P_np[0:n, :])
    elif stg == 'near':
...
def gabriel_graph(P: np.ndarray) -> np.ndarray:
    r"""
    Perform delaunay triangulation on point set P.
    ...
    if n < 3:
        ...
        try:
            ...
            # pruning the non-Gabriel edges
            edge_pair_idx = np.stack(A.nonzero()).T 
            center = d.points[edge_pair_idx]
            m = (center[:, 0, :] + center[:, 1, :])/2
            # Find the radius sphere between each pair of nodes
            r = sp.sqrt(sp.sum((center[:, 0, :] - center[:, 1, :])**2, axis=1))/2
            ...
            A[edge_pair_idx[g][:,0], edge_pair_idx[g][:, 1]] = 0

        ...
        for i, inputs in enumerate(dataloader[set]):
            ...
            for j in range(inputs['batch_size']):
                ...
                for idx, pred_perm in enumerate(pred_perms):
                    ...
                    plot_2graph_helper(data1[j], data2[j], P1_gt[j], P2_gt[j], n1_gt[j], n2_gt[j], ax, colorset, pred_perm[j], perm_mat[j], names[idx+1])
...
def plot_2graph_helper(imgsrc, imgtgt, Psrc, Ptgt, nsrc, ntgt, ax, colorset, pmat, gt_pmat, method=""):

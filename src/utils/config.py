...
# score regularization ratio for QAP problem
__C.TRAIN.SCORE_REG_RATIO = 0.

# Regularization Level
__C.TRAIN.REG_LEVEL = 1

# Regularization ratio
__C.TRAIN.REG_RATIO = 1.

# whether to perform AT or our defense strategy
__C.TRAIN.MODE = 'at'

# Burn-in period 
__C.TRAIN.BURN_IN_PERIOD = 1

# Whether to synchronize the min-max optimization way
__C.TRAIN.SYNC_MINMAX = 0

# whether to keep BN layer static during crafting attacks 
__C.TRAIN.BN_EVAL = False

# attack only node features or edge features or both
__C.TRAIN.FEAT_ATT_MODE = 'both'
...
__C.EVAL.NUM_EPOCH = 4
 
# Whether mask the non-diagnol elements in the Affinity Matrix
__C.EVAL.AFF_MASK = 0

# How to perform attacks during evaluation
__C.EVAL.MODE = 'clean'
...
# pixel bound during training
__C.ATTACK.EPSILON_FEATURE = 8.

# keypoint locality bound during training
__C.ATTACK.EPSILON_LOCALITY = 8.

# pixel bound during evaluation
__C.ATTACK.EVAL_EPSILON_FEATURE = 8.

# locality bound during evaluation
__C.ATTACK.EVAL_EPSILON_LOCALITY = 8.

# num of step size during training
__C.ATTACK.ALPHA = 1.

# step size during evaluation
__C.ATTACK.EVAL_ALPHA = 0.25

# num of step during training
__C.ATTACK.STEP = 1

# step size during evaluation
__C.ATTACK.EVAL_STEP = 10

# num of random restarts 
__C.ATTACK.RESTARTS = 1

# early stop ratio 
__C.ATTACK.EARLY_STOP_RATIO = 0.

# ratio for momentum-based attack 
__C.ATTACK.MU = 0.0

# which way to iteratively optimize attacks
__C.ATTACK.TYPE = 'pgd'

# which part of inputs being attacked
__C.ATTACK.OBJ_TYPE = 'pixel'

# attack objective loss function 
__C.ATTACK.LOSS_FUNC = 'perm'
__C.ATTACK.INV = False
__C.ATTACK.BLACK = False

# attack params for warmup 
__C.ATTACK2.TYPE = 'pgd'
__C.ATTACK2.OBJ_TYPE = 'pixel'
__C.ATTACK2.STEP = 1
...
# DATETIME for loading the specific model
__C.DATETIME = None
...
def get_output_dir_new(prefix, now_day=None, exp_name=None):
    """
    Return the directory where experimental artifacts are placed.
    :param model: model name
    :param dataset: dataset name
...
def cfg_from_file(filename):
    ...
    if 'VICTIM_MODULE' in yaml_cfg and yaml_cfg.VICTIM_MODULE not in __C:
        model_cfg_module = '.'.join(yaml_cfg.VICTIM_MODULE.split('.')[:-1] + ['model_config'])
        mod = importlib.import_module(model_cfg_module)
        __C.update(mod.model_cfg)

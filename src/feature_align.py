...
def detect_bound(P: Tensor, ns_t: Tensor, ori_size: tuple, device=None, perturb_type='node') -> Tensor:
    r"""
    Return the positions of the four corner points over the whole batch 
    ...
    :param ori_size: size of the original image. Since the point coordinates are in the scale of the original image
     size, this parameter is required.
    :param device: output device. If not specified, it will be the same as the input
    :return: :math:`(b\times n \times 4)` extracted feature vectors
    """
    if device is None:
        device = torch.device("cuda:0" if torch.cuda.is_available() else "cpu")
    if perturb_type == 'node':
        batch_num = P.shape[0]
        channel_num = 512
        feat_size = torch.as_tensor([32, 32], dtype=torch.float32, device=device)
    elif perturb_type == 'edge':
        batch_num = P.shape[0]
        channel_num = 512
        feat_size = torch.as_tensor([16, 16], dtype=torch.float32, device=device)
    else:
        raise ValueError('No matched perturbed type. Optional: node|edge')

    n_max = P.shape[1]

    ori_size = torch.tensor(ori_size, dtype=torch.float32, device=device)
    F = torch.zeros(batch_num, n_max, 6, dtype=torch.float32, device=device)
    # for idx, feature in enumerate(raw_feature):
    for idx in range(batch_num):
        n = ns_t[idx]
        # feat_size = torch.as_tensor(feature.shape[1:3], dtype=torch.float32, device=device)
        _P = P[idx, 0:n]
        interp_2d_1(_P, ori_size, feat_size, out=F[idx, 0:n, :])
    return F

def interp_2d_1(P: Tensor, ori_size: Tensor, feat_size: Tensor, out=None, device=None) -> Tensor:
    r"""
    Interpolate in 2d grid space. z can be 3-dimensional where the first dimension is feature dimension.
    ...
    step = ori_size / feat_size
    if out is None:
        out = torch.zeros((P.shape[0], 6), dtype=torch.float32, device=device)
    for i, p in enumerate(P):
        p = (p - step / 2) / ori_size * feat_size
        out[i, :] = bilinear_interpolate_1(feat_size, p[0], p[1])
    return out

def bilinear_interpolate_1(feat_size: Tensor, x: Tensor, y: Tensor, device=None):
    r"""
    Bi-linear interpolate 3d feature map to 2d coordinate (x, y).
    The coordinates are at the same scale of :math:`w\times h`.
    ...
    x0 = torch.clamp(x0, 0, feat_size[1] - 1)
    x1 = torch.clamp(x1, 0, feat_size[1] - 1)
    y0 = torch.clamp(y0, 0, feat_size[0] - 1)
    y1 = torch.clamp(y1, 0, feat_size[0] - 1)
    ...
    return torch.tensor([x0, x1, y0, y1, x, y])